# coding:utf-8
# --------------------------------------------------------
# RFA-Net with VGG-Net for Faster RCNN 
# Licensed under The MIT License [see LICENSE for details]
# Written by Yangguang Zhu, based on code from Xinlei Chen
# --------------------------------------------------------
...
from model.faster_rcnn.faster_rcnn_rfa_net  import _fasterRCNN
...
class Reconstruction_D_pixel(nn.Module):

    def __init__(self):
        super(Reconstruction_D_pixel, self).__init__()
        self.adapt_pooling = nn.AdaptiveAvgPool2d((150, 150))
        self.Reconstruction = Reconstruction(dim=256, n=150 * 150, ratio=8, K=6)
        self.Reconstruction_bn = nn.BatchNorm2d(256)

    def forward(self, x):
        x = self.adapt_pooling(x)
        x = F.relu(self.Reconstruction_bn(self.Reconstruction(x)) + x)
        return x
...
class vgg16(_fasterRCNN):
    ...
    def _init_modules(self):
        ...
        self.Reconstruction_D_pixel_s = Reconstruction_D_pixel()
        self.Reconstruction_D_pixel_t = Reconstruction_D_pixel()
