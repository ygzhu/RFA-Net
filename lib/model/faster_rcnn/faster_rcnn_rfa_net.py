...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, target=False, eta=1.0, training=True):

        if not self.training:
            return self.inference(im_data, im_info, gt_boxes, num_boxes, target, eta, training)
        batch_size = im_data.size(0)

        im_info = im_info.data
        gt_boxes = gt_boxes.data
        num_boxes = num_boxes.data
        base_feat1_ = self.RCNN_base1(im_data)

        if training:
            if not target:
                base_feat1 = self.Reconstruction_D_pixel_s(base_feat1_)
            else:
                base_feat1 = self.Reconstruction_D_pixel_t(base_feat1_)
        else:
            base_feat1 = self.Reconstruction_D_pixel_t(base_feat1_)

        if self.lc:
            # TODO whether use context vector for pixel level
        ...
        base_feat = self.RCNN_base2(base_feat1_)

        if self.gc:
            # TODO whether use context vector for global level
        ...
        if target:
            return rois, rpn_loss_cls, rpn_loss_bbox, d_pixel, domain_p
    ...
    def inference(self, im_data, im_info, gt_boxes, num_boxes, target=False, eta=1.0, training=True):
        batch_size = im_data.size(0)
        ...
        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, d_pixel, None  # ,diff
