...
from roi_data_layer.roibatchLoader import roibatchLoader, roibatchLoader_umt
...
def get_dataloader(imdb_name, args, sequential=False, s_train_size=None, augment=False):
    ...
    dataset = roibatchLoader_umt(roidb, ratio_list, ratio_index, args.batch_size, \
...
# def local_attention(features, d_pixel):
#     """
#     利用domain label作为一个attetion，采用res的结构
#     :param features:resnet网络layer1提取的特征
#     :param d_pixel: local域判别器的输出
#     :return:
#     """
#     d_pixel = d_pixel.clamp(1e-6, 1)
#     print(d_pixel.shape)
#     print(features.shape)
#
#     features_new = (1 + d_pixel) * features
#
#     return features_new
...
def cv_text(im, bbox, class_name, score):
    """
    绘制可视化检测结果,可根据文字的长短大小调整文字显示范围
    :param im: 
    :param bbox: 
    :param class_name: 
    :param score: 
    ...
    # 绘制矩形框
    cv2.rectangle(im, bbox[0:2], bbox[2:4], color=(0, 255, 0), thickness=2)
    # 计算文本的宽高,baseLine
...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        score = dets[i, -1]
        if score > thresh:
            im = cv_text(im, bbox, class_name, score)
